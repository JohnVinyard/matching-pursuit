...
class Discriminator(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder = Encoder(is_disc=True)
...
ae_optim = Adam(chain(encoder.parameters(), decoder.parameters()),
                lr=1e-4, betas=(0, 0.9))
...
def train_gen(batch):
    ...
    # get disc loss
    _, r_features = disc(batch, batch)
    _, f_features = disc(fake_feat, batch)

    feature_loss = 0
    for i in range(len(r_features)):
        feature_loss = feature_loss + \
            torch.abs(r_features[i] - f_features[i]).sum()
    feature_loss = feature_loss * 1e-4
...
@readme
class MultiresolutionAutoencoderWithActivationRefinements15(object):
