...
class TransferFunctionModel(nn.Module):
    ...
    def __init__(
        ...
        # bank *= np.linspace(1, 0, self.n_samples) ** 20

        self.atoms = nn.Parameter(torch.zeros(
            self.n_atoms, self.atom_size).uniform_(-1, 1))
        self.transfer = nn.Parameter(torch.zeros(
            self.n_transfers, self.transfer_size).uniform_(-1, 1) * torch.linspace(1, 0, self.transfer_size)[None, ...] ** 20)
    ...
    def forward(self, x):
        ...
        atom_amps = self.to_atom_amp.forward(latents) ** 2
