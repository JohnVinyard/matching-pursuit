...
class Model(nn.Module):
    def __init__(self):
        ...
        self.encoder = UNet(1024, return_latent=alternate_model)
        ...
        if alternate_model:
            self.planner = Planner(4096, 128, context_dim, 256, mixer_planning=False)
            self.to_low_dim_context = nn.Linear(1024, context_dim)
    ...
    def generate(self, encoded, one_hot, packed, dense):
        ...
        if alternate_model:
            embeddings = self.planner.forward(one_hot, packed, dense)
        else:
    ...
    def forward(self, x):
        if alternate_model:
            ...
            dense = self.to_low_dim_context(z)
        else:
...
#         # generate context latent
#         z = torch.zeros(1, context_dim).normal_(0, 0.5)
        
#         # generate events
#         events = torch.zeros(1, 4096, 128).uniform_(0, 4)
#         # events = F.avg_pool2d(events, (7, 7), (1, 1), (3, 3))
#         # events = events.view(1, 4096, 128)
...
# @readme
# class L0Norm(BaseExperimentRunner):
