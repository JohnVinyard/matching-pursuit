...
ae_optim = Adam(decoder.parameters(), lr=1e-4, betas=(0, 0.9))
...
def train_disc(batch):
    ...
    # get disc loss
    _, rj = disc(batch)
    encodings, fj = disc(fake_feat)

    # encoder should learn to invert generator
    latent_loss = F.mse_loss(encodings, z)

    # disc should learn to differentiate real and fake
    # based on latent code
    adv_loss = least_squares_disc_loss(rj, fj)

    loss = (adv_loss * 1) + (latent_loss * 0.33)
...
@readme
class MultiresolutionAutoencoderWithActivationRefinements16(object):
