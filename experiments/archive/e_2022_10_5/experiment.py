...
class GenerateSpectrogram(nn.Module):
    def __init__(self):
        ...
        self.to_spec = ConvUpsample(
            ...
            exp.n_frames, 
            mode='learned', 
            out_channels=exp.scale.n_bands)
    ...
    def forward(self, times, envs, transfer_functions):
        ...
        x = self.to_spec(x).view(-1, n_events, 128, 128)
...
grad_optim = optimizer(synth_grad, lr=1e-4)
...
def train_synthetic_grad(batch):
    ...
    with torch.no_grad():
        ...
        # spec = stft(recon, 512, 256, pad=256).view(-1, 128, 257)
        spec = exp.pooled_filter_bank(recon)
...
def train_model(batch):
    ...
    # compute real spec
    # spec = stft(batch, 512, 256, pad=256).view(-1, 128, 257)
    spec = exp.pooled_filter_bank(batch)
