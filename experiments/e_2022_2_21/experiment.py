...
from loss.least_squares import least_squares_disc_loss, least_squares_generator_loss, squared_gan_loss
...
class EncoderBranch(nn.Module):
    ...
    def __init__(self, band_size, periodicity_feature_size, is_disc=False):
        ...
        self.context = nn.Conv1d(network_channels * 2 if is_disc else network_channels, network_channels, 3, 1, 1)
        ...
        if is_disc:
            ...
            self.judge = nn.Sequential(
                ...
                nn.LeakyReLU(0.2),
                nn.Conv1d(network_channels, 1, 3, 1, 1)
            )
    ...
    def forward(self, x, conditioning=None):
        ...
        if self.is_disc:
            # conditioning = conditioning * condition_adjust[self.band_size]
        ...
        if self.is_disc:
            x = torch.cat([x, spec_context], dim=-1)
...
class Encoder(nn.Module):
    def __init__(self, is_disc=False):
        ...
        self.is_disc = is_disc
        if is_disc:
            self.judge = LinearOutputStack(
                network_channels, 3, out_channels=1, in_channels=network_channels * 6)
    ...
    def forward(self, x, conditioning=None):
        ...
        if self.is_disc:
            x, features, judgements = self.shell(feat, conditioning)
        else:
        ...
        if self.is_disc:
            r = self.judge(x)
            # judgements = [x.permute(0, 2, 1) for x in judgements]
            # r = torch.cat([*judgements, r], dim=-1)
            return r, features[0]
    ...
    def __init__(self, twod=False):
        ...
        if self.twod:
            self.net = nn.Sequential(
                ...
                nn.ConvTranspose2d(4, 4, (4, 1), (2, 1), (1, 0)),
                # (1, 64, 32)
            )
        else:
            self.net = nn.Sequential(
...
def train_gen(batch):
    ...
    for i in range(len(r_features)):
        adv_loss = adv_loss + torch.abs(r_features[i] - f_features[i]).sum()
    adv_loss = adv_loss * 0.01

    pif_loss = 0
    for k, v in batch.items():
        pif_loss = pif_loss + torch.abs(fake_feat[k] - v).sum()
    pif_loss = pif_loss * 1e-6
    

    loss = pif_loss + adv_loss

    print('G', pif_loss.item(), adv_loss.item())
...
@readme
class MultiresolutionAutoencoderWithActivationRefinements12(object):
