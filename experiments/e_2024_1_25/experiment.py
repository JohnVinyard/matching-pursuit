...
class UNet(nn.Module):
    def __init__(self, channels, return_latent=False):
        ...
        self.latent_proj = nn.Linear(4096, 1024)
...
class Model(nn.Module):
    ...
    def encode(self, x):
        ...
        if x.shape[-1] == exp.n_samples:
            x = stft(x, 2048, 256, pad=True)

        x = x.view(batch_size, 128, 1025)[..., :1024].permute(0, 2, 1)
    ...
    def forward(self, x, precomputed_dense=None):
        encoded = self.encode(x)
        
        if precomputed_dense is not None:
            dense = precomputed_dense
        else:
        ...
        return final, encoded, imp, dense
...
def train_single_channel(batch, i):

    b = batch.shape[0]
    
    reported_loss = 0
    
    residual = batch.clone().detach()
    residual = stft(residual, 2048, 256, pad=True)
    
    
    output_recon = torch.zeros_like(batch)
    output_encoding = torch.zeros(batch.shape[0], 4096, 128, device=batch.device)
    ...
    for i in range(16):
        ...
        recon, encoded, imp, dense = model.forward(residual)
        
        output_encoding = output_encoding + encoded
        
        output_recon = output_recon + recon
        
        recon_spec = stft(recon, 2048, 256, pad=True)
        
        start_norm = torch.norm(residual, dim=(-1, -2), p=1)
        
        residual = residual - recon_spec
        
        end_norm = torch.norm(residual, dim=(-1, -2), p=1)
        loss = (-(start_norm - end_norm)).sum()
        
        reported_loss = reported_loss + loss
    
        loss.backward()
        optim.step()
    
    
    recon = max_norm(output_recon)
    encoded = max_norm(output_encoding)
    
    return reported_loss, recon, encoded
...
def train(batch, i):
    ...
    for i in range(16):
        ...
        recon, encoded, imp, dense = model.forward(residual, precomputed_dense=ctxt)
...
class IterativeDecomposition(BaseExperimentRunner):
    ...
    def run(self):
        for i, item in enumerate(self.iter_items()):
            ...
            l, r, e = train_single_channel(item, i)
