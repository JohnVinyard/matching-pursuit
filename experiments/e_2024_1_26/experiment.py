...
class Analyzer(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.channels = channels
        self.net = AntiCausalStack(channels, 2, [1, 2, 4, 8, 16, 32, 64, 1])
        self.up = nn.Conv1d(channels, 4096, 1, 1, 0)
...
def train(batch, i):
    ...
    with torch.no_grad():
        ...
        # generate context latent
        z = torch.zeros(1, context_dim).normal_(-9, 80)
        
        # generate events
        events = torch.zeros(1, 4096, 128).uniform_(0, 266)
        # events = F.avg_pool2d(events, (7, 7), (1, 1), (3, 3))
        # events = events.view(1, 4096, 128)
...
@readme
class AntiCausalConv(BaseExperimentRunner):
