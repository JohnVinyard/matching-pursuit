...
class Model(nn.Module):
    def __init__(self):
        ...
        self.encoder = AntiCausalStack(analysis_channels, kernel_size=2, dilations=[1, 2, 4, 8, 16, 32, 64, 1])
        
        # self.encoder = UNet(1024, return_latent=False, is_disc=False)
        
        self.to_event_vectors = nn.Conv1d(analysis_channels, context_dim, 1, 1, 0)
        self.to_event_switch = nn.Conv1d(analysis_channels, 1, 1, 1, 0)
        
    
        total_atoms = 256
    
        self.embed_context = nn.Linear(total_atoms, 256)
        self.embed_one_hot = nn.Linear(total_atoms, 256)
        self.embed_latent = nn.Linear(analysis_channels, context_dim)
        

        self.imp = GenerateImpulse(256, 128, impulse_size, 16, n_events)
        
        self.to_f0 = nn.Linear(256, 3)
        self.resonance = F0Resonance(n_octaves=32, n_samples=exp.n_samples)
        ...
        self.atom_bias = nn.Parameter(torch.zeros(total_atoms).uniform_(-1, 1))
    ...
    def generate(self, vecs, scheduling):
        ...
        # resonances
        res_x = self.to_f0(embeddings)
        
        mixed = self.resonance.forward(
            torch.sigmoid(res_x[..., :1]) ** 2, 
            res_x[..., 1:2], 
            torch.sigmoid(res_x[..., 2:3]) * 2)
...
@readme
class GumbelSoftmaxScheduling(BaseExperimentRunner):
