...
class BandUpsample(nn.Module):
    ...
    def __init__(self, band_size):
        ...
        layers = int(np.log2(band_size) - np.log2(32))
        self.net = nn.Sequential(*[nn.Sequential(
            nn.Upsample(scale_factor=2, mode='nearest'),
            nn.Conv1d(network_channels, network_channels, 7, 1, 3)
        ) for _ in range(layers)])

        self.final = nn.Conv1d(network_channels, 64, 7, 1, 3)
    ...
    def forward(self, x):
        ...
        fb, _, _ = feature.banks[self.band_size]
        x = F.pad(x, (0, 1))
...
@readme
class MultiresolutionFilterBankAutoencoder(object):
