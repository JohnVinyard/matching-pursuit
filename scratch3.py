...
class Instrument3(nn.Module):
    ...
    def __init__(
        ...
        self.hyper = HyperNetworkLayer(
            shape_channels, 16, channels, encoding_channels)
        
    def _pos_encoding(self, n_samples: int):
        """Returns a filterbank with periodic functions
        """
        freqs = torch.linspace(0.00001, 0.49, steps=self.encoding_channels)
    ...
    def forward(
        ...
        # TODO: This could be a "deep", non-linear transformation,
        # rather than a linear one
        # transformed = energy @ transforms
        # torch.relu(transformed)
        
        # transformed = transformed.permute(0, 1, 3, 2).view(batch * n_events, self.encoding_channels, frames)
        # transformed = F.interpolate(transformed, size=self.n_samples, mode='linear')
        # transformed = transformed.view(batch, n_events, self.encoding_channels, self.n_samples)
...
def tryout_instrument3():
    ...
    # transforms = torch.zeros(batch_size, n_events, control_plane, encoding_channels).uniform_(-1, 1)
