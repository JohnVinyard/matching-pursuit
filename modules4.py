...
class ResidualUpscale(nn.Module):
    def __init__(self, channels, heads=4, use_context=False):
        super().__init__()
        self.use_context = use_context

        if use_context:
            self.context = LinearOutputStack(
                channels, 3, in_channels=channels * 3)
...
class ConvExpander(nn.Module):
    def __init__(self, channels, max_atoms):
        ...
        self.net = nn.Sequential(*[
            ResidualUpscale(channels, heads=i + 1, use_context=True)
...
class TripletConvExpander(nn.Module):
    def __init__(self, channels, max_atoms):
        super().__init__()
        self.channels = channels
        self.max_atoms = max_atoms
        ...
        # self.attn = nn.Sequential(*[MultiHeadAttention(channels * 3, 4) for _ in range(n_layers)])

        self.atom_expander = nn.Sequential(*[
            ResidualUpscale(channels, heads=i + 1, use_context=i > 0)
            for i in range(n_layers)
        ])
        self.pos_expander = nn.Sequential(*[
            ResidualUpscale(channels, heads=i + 1, use_context=i > 0)
            for i in range(n_layers)
        ])
        self.mag_expander = nn.Sequential(*[
            ResidualUpscale(channels, heads=i + 1, use_context=i > 0)
            for i in range(n_layers)
        ])
    ...
    def forward(self, x):
        ...
        i = 0
        for al, pl, ml in zip(self.atom_expander, self.pos_expander, self.mag_expander):
            a = al(x)
            p = pl(x)
            m = ml(x)
...
class Generator(nn.Module):
    def __init__(
        ...
        self.expander = TripletConvExpander(channels, max_atoms)
    ...
    def forward(self, x):
        ...
        atom_encodings, pos_encodings, mag_encodings = self.expander(x)

        atoms = self.atoms(atom_encodings)
        p = sine_one(self.pos(pos_encodings))
        m = sine_one(self.mag(mag_encodings))
